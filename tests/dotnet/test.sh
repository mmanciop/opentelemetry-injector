...
docker buildx build -q --platform linux/${arch} --build-arg TARGETARCH=${arch} -o type=image,name=otelinject-test-dotnet,push=false .
OUTPUT=$(docker run --platform linux/${arch} --rm otelinject-test-dotnet)
echo "========== OUTPUT =========="
echo "$OUTPUT"
echo "============================"
...
OUTPUT_BASH=$(docker run --platform linux/${arch} --rm otelinject-test-dotnet /usr/bin/env)
echo "======= BASH OUTPUT ========"
echo "$OUTPUT_BASH"
echo "============================"
