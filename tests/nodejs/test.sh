...
docker buildx build -q --platform linux/${arch} --build-arg BASE=$BASE -o type=image,name=otelinject-test-nodejs,push=false .
OUTPUT=$(docker run --platform linux/${arch} --rm otelinject-test-nodejs)
echo "========== OUTPUT =========="
echo "$OUTPUT"
echo "============================"
